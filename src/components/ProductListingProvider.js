...
export function ProductListingProvider(props) {
    ...
    const notifyTC = (till) => toast.warn((lang === "AZ" && ` ${till/60} dəqiqə sonra yenidən cəhd edin` || lang === "EN" && `Please try again later after ${till/60} minutes` || lang === "RU" && `Пожалуйста, попробуйте еще раз позже через  ${till/60} минут`));
    const notifyAuth = () => toast.error(
        <div className="authCont">
          <p className="title">{ <> {(lang === "AZ" && "Hesabınızı təsdiqləyin !" || lang === "EN" && `Confirm your account!` || lang === "RU" && `Подтвердить свой аккаунт!`)}</>} </p>  
          <button onClick={() => smsHandle()} className='phoneAuth'>{(lang === "AZ" && `Telefonla təsdiqləmək ` || lang === "EN" && `Confirm by phone` || lang === "RU" && `Подтвердить по телефону`)}</button>
          <button onClick={() => logout()} className='phoneAuth'>{(lang === "AZ" && `Çıxış` || lang === "EN" && `Log out` || lang === "RU" && `Выйти`)}</button>
          <p>{tillCount > 0 &&  <>{(lang === "AZ" && ` ${tillCount/60} dəqiqə sonra yenidən cəhd edin` || lang === "EN" && `Please try again later after ${tillCount/60} minutes` || lang === "RU" && `Пожалуйста, попробуйте еще раз позже через  ${tillCount/60} минут`)}</> }</p>
        </div>
        , {
            position: "top-right",
        autoClose:8000,
