...
function Certificates(props) {
    ...
    return (
        <div className="certificates">
            { props.Product?.cert_pdf?.length > 0 ? <button style={bgImage} href={`https://nehra.az/${props.Product.cert_pdf}`} onClick={handleOpen} className="certificate"><div className="overlay"><WallpaperIcon/></div></button> : <p className='certNo'>{(lang === "AZ" && "Sertifikat mövcud deyil.") || (lang === "EN" && `There are no comments for this product`) || (lang === "RU" && `К этому продукту нет комментариев`)}</p>}
